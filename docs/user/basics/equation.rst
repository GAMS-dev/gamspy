...
listing can be filtered with ``elements`` argument and the number of equations returned can be limited with ``n`` argument.
...
One can alternatively filter certain equations by using the ``elements`` argument. For example, if one only wants to see 
the equations for monterrey market, they can provide the elements as follows: ::
  ...
  ...
  model.solve(options=Options(equation_listing_limit=100))
  print(mr.getEquationListing(elements=[[], ['monterrey']]))

``elements`` argument is a list of lists where each list specifies the elements to be gathered. 
...
.. note::
  ...
  Length of the ``elements`` argument must be equal to the dimension of the equation.