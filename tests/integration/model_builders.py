"""Model definitions shared by the ``toGams`` and ``toLatex`` conversion tests."""
...
def lp_transport(m: Container) -> Model:
    """LP: the canonical transport model."""
    i = Set(
        m,
        name="i",
...
def dnlp_inscribedsquare(m: Container) -> Model:
    """DNLP: largest square inscribed in a closed curve."""
...
def mcp_qp6() -> Model:
    """MCP: the QP6 model, loaded from qp6.gdx."""
...
def minlp_minlphix(m: Container) -> Model:
...
def mip_cutstock(m: Container) -> Model:
    """MIP: cutting stock master problem."""
    i = Set(
        m,
        "i",
...
def nlp_weapons(m: Container) -> Model:
    """NLP: weapons assignment."""
...
def qcp_EDsensitivity(m: Container) -> Model:
    """QCP: economic dispatch sensitivity model."""
