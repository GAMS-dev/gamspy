...
def main():
    ...
    phi = Parameter(
        m, "phi", domain=i, description="transformation elasticity parameter"
    )
    alpha = Parameter(
        m, "alpha", domain=i, description="share parameter in utility func."
    )
    beta = Parameter(
        m,
        "beta",
    ...
    lambda_renamed = Parameter(
        m, "lambda", domain=i, description="investment demand share"
    )
    deltam = Parameter(
        m, "deltam", domain=i, description="share par. in Armington func."
    )
    deltad = Parameter(
        m, "deltad", domain=i, description="share par. in Armington func."
    )
    gamma = Parameter(m, "gamma", domain=i, description="scale par. in Armington func.")
    xid = Parameter(
        m, "xid", domain=i, description="share par. in transformation func."
    )
    xie = Parameter(
        m, "xie", domain=i, description="share par. in transformation func."
    )
    theta = Parameter(
        m,
        "theta",
    ...
    gamma[i] = Q0[i] / (
        ((deltam[i] * (M0[i] ** eta[i])) + (deltad[i] * (D0[i] ** eta[i])))
        ** (1 / eta[i])
    )
    xie[i] = (E0[i] ** (1 - phi[i])) / (
        (E0[i] ** (1 - phi[i])) + (D0[i] ** (1 - phi[i]))
    )
    xid[i] = (D0[i] ** (1 - phi[i])) / (
        (E0[i] ** (1 - phi[i])) + (D0[i] ** (1 - phi[i]))
    )
    ...
    assert math.isclose(stdcge.objective_value, 25.508490012515818), (
        stdcge.objective_value
    )
    ...
    assert math.isclose(stdcge.objective_value, 26.092634381288686), (
        stdcge.objective_value
    )
