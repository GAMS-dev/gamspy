...
pv1 = Set(container, "pv1", domain=[pv], description="provinces")
...
t1 = Set(container, "t1", domain=["*"], description="sub zones by gw quality")
r1 = Set(container, "r1", domain=["*"], description="resources")
dc = Set(container, "dc", domain=[r1], description="characteristics of canal command")
...
ai = Set(container, "ai", domain=["*"], description="animals input output")
...
repco = Parameter(container, "repco", domain=[], description="reproductive coefficient")
gr = Parameter(
    container,
    "gr",
    domain=[],
...
pvcnl = Set(container, "pvcnl", domain=[pv, cnl], description="province to canals map")
gwfg = Set(
    container,
    "gwfg",
    domain=[cnl, sa, g],
...
drc = Parameter(container, "drc", domain=[], description="run-off portion of rainfall")
the1 = Parameter(
    container,
    "the1",
    domain=[],
...
nn = Set(container, "nn", domain=[n, n1], description="water flow system node to node")
ni = Set(container, "ni", domain=[n, i], description="node to rim station inflow map")
...
rulelo = Parameter(container, "rulelo", domain=[n, m], description="lower rule curve")
ruleup = Parameter(container, "ruleup", domain=[n, m], description="upper rule curve")
revapl = Parameter(
    container,
    "revapl",
    domain=[n, m],
...
prices = Parameter(container, "prices", domain=[ps, cq, p3], description="1988  prices")
finsdwtpr = Parameter(
    container,
    "finsdwtpr",
    domain=[c, ps, "*"],
...
cnl1 = Set(container, "cnl1", domain=[cnl], description="canals excluding nwfp canals")
postt = Parameter(
    container,
    "postt",
    domain=["*", "*"],
...
cm = Set(container, "cm", domain=[cq], description="comodities which could be imported")
ex = Set(
    container,
    "ex",
    domain=[z, g],
...
eseedp = Parameter(container, "eseedp", domain=[c], description="economic seed price")
ewage = Parameter(
    container,
    "ewage",
    domain=[m],
...
fert[p2, c, z] = fert[p2, c, z] * ((1 + (fertgr[c] / 100)) ** (isr.val - baseyear[...]))
...
efr[cnl, m] = (((1 - drc[...]) - (1 - comdef[isr, "flde", cnl])) * rain[cnl, m]) / 12
...
explimit[z, cq] = (natexp[cq] * Sum(pv.where[pvz[pv, z]], exppv[pv, cq])) * expzo[z, cq]
explimit[z, cq] = (
...
canalwrtz[z, g, m] = Sum(
    Domain(cnl, sa).where[(zsa[z, cnl, sa].where[gwfg[cnl, sa, g]])],
    (((divpost[cnl, m] * subdef[sa, cnl]) * comdef[isr, "ceff", cnl]) * wceff[cnl, m]),
)
...
qcombal[ex[z1, g], q] = (Sum(a, (iolive[a, z1, q] * animal[z1, g, a])) / 1000) == prodt[
    z1, g, q
]
...
protein[ex[z1, g], sea] = Sum(a, (iolive[a, z1, "dp"] * animal[z1, g, a])) <= (
    (
        ...
        + ((Sum(m, slkland[z1, g, m]) * graz[z1, sea]) * sconv["dp", "rabi", "rab-fod"])
    )
...
waterbaln[ex[z1, g], m] = (
    Sum(
        ...
        (
            gp_math.Max(
                (wnr[c, z1, t, s, w, m] - (subirrz[z1, m] * land[c, z1, t, s, w, m])), 0
            )
            * x[z1, g, c, t, s, w]
        ),
    )
    ...
    (((tweffz[z1, m] * (tw[z1, m].where[gf[g]])) + gwt1[z1, g, m]) + artwater[z1, g, m])
    + wdivrz[z1, g, m]
)
...
watalcpro[pv2, sea] = ((protarb[pv2, sea] * (1 - tolpr[...])) * tcdivsea[sea]) <= prsea[
    pv2, sea
]
