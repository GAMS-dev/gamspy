...
def main():
    ...
    while mu_target <= MAX_MU.records.value[0]:
        ...
        output_csv += (
            f"{str(MinCVaR.status).split('.')[-1]},{VaR.records.level.round(3)[0]},{round(MinCVaR.objective_value, 3)},{round(MU_TARGET.records.value[0] * Budget.records.value[0],3)},"
        )
    ...
    FrontierHandle = open("CVaRFrontiers.csv", "w", encoding="UTF-8")
