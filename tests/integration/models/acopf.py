...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        help=(
            "Whether to turn off wind turbines. Can only be used with PrimeMover,pm_WT."
        ),
    )
    ...
    c_LinePji[i, j, c].where[branchstatus[i, j, c]] = V_LineP[j, i, c] == g[i, j, c] * (
    ...
    ) - Pd[i] == Sum(Domain(j, c).where[branchstatus[i, j, c]], V_LineP[i, j, c]) + Sum(
    ...
    ) - Qd[i] == Sum(Domain(j, c).where[branchstatus[i, j, c]], V_LineQ[i, j, c]) + Sum(
