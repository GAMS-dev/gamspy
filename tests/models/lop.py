...
def main():
    ...
    # l[root, s, s1, s2].where[~ll[root, s]] = Number(1)
    # lr[root, s, s1, r].where[~ll[root, s]] = Number(0)

    # l[root, s, s1, s2].where[l[root, s, s2, s1] & rt[s1, s2]] = Number(1)
    # l[root, s, s1, s2].where[~rt[s1, s2]] = Number(0)
    ...
    # solsum["DT", ll, "cars"].where[phi.l[ll]] = mincars + Card(ac) - 1
