...
def main():
    ...
    # Equation
    nlpobj = Equation(cont, name="nlpobj", type="eq")
    tctrlo = Equation(cont, name="tctrlo", type="geq", domain=[i, j])
    lmtdlo = Equation(cont, name="lmtdlo", type="geq", domain=[i])
    lmtdsn = Equation(cont, name="lmtdsn", type="eq", domain=[i])
    tempset = Equation(cont, name="tempset", type="leq", domain=[i])
    artrex1 = Equation(cont, name="artrex1", type="leq", domain=[i])
    artrex2 = Equation(cont, name="artrex2", type="leq", domain=[i])
    material = Equation(cont, name="material", type="eq", domain=[m])
    feed = Equation(cont, name="feed", type="eq")
    matlog = Equation(cont, name="matlog", type="leq", domain=[i])
    duty = Equation(cont, name="duty", type="eq", domain=[i])
    rebcon = Equation(cont, name="rebcon", type="eq", domain=[i, j])
    conheat = Equation(cont, name="conheat", type="eq", domain=[i])
    rebheat = Equation(cont, name="rebheat", type="eq", domain=[j])
    dtminlp = Equation(cont, name="dtminlp", type="leq", domain=[j])
    dtminc = Equation(cont, name="dtminc", type="leq", domain=[i])
    trtcdef = Equation(cont, name="trtcdef", type="eq", domain=[i, j])
    dtmincr = Equation(cont, name="dtmincr", type="leq", domain=[i, j])
    dtminex = Equation(cont, name="dtminex", type="leq", domain=[j])
    hxclog = Equation(cont, name="hxclog", type="leq", domain=[i, j])
    hxhulog = Equation(cont, name="hxhulog", type="leq", domain=[hu, j])
    hxculog = Equation(cont, name="hxculog", type="leq", domain=[i, cu])
    qcqrlog = Equation(cont, name="qcqrlog", type="leq", domain=[i])

    # these are the pure binary constraints of the minlp
    sequen = Equation(cont, name="sequen", type="eq", domain=[m])
    lead = Equation(cont, name="lead", type="eq")
    limutil = Equation(cont, name="limutil", type="leq", domain=[j])
    hidirect = Equation(cont, name="hidirect", type="leq", domain=[i, j])
    heat = Equation(cont, name="heat", type="leq", domain=[i])
