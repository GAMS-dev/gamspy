...
def main():
    m = Container(load_from=str(Path(__file__).parent.absolute()) + "/spatequ.gdx")
    ...
    P2R3_Linear = Model(
        m, name="P2R3_Linear", equations=[DEM, SUP, SDBAL, PDIF, TRANSCOST, SX, DX]
    )
    ...
    P2R3_NonLinear = Model(
        m, name="P2R3_NonLinear", equations="P2R3_Linear, DEMINT, SUPINT, OBJECT"
    )
    P2R3_MCP = Model(m, name="P2R3_MCP", equations="DEM, SUP, IN_OUT.P, DOM_TRAD.X")
