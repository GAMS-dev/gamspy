...
class NNFormulationSuite(unittest.TestCase):
    def setUp(self):
        self.m = Container()
        self.w1 = np.array(
            [
        ...
        self.b1 = np.array([0.56258535, 0.53484524])
        self.inp = np.array(
            [
                [
                    [
    ...
    def test_conv2d_bad_init(self):
        # in channel must be integer
        self.assertRaises(ValidationError, Conv2d, self.m, 2.5, 4, 3)
        self.assertRaises(ValidationError, Conv2d, self.m, "2", 4, 3)
        # out channel must be integer
        self.assertRaises(ValidationError, Conv2d, self.m, 2, 4.1, 3)
        self.assertRaises(ValidationError, Conv2d, self.m, "2", 4.1, 3)
        # in channel must be positive
        self.assertRaises(ValidationError, Conv2d, self.m, -4, 4, 3)
        # out channel must be positive
        self.assertRaises(ValidationError, Conv2d, self.m, 4, -4, 3)
        # kernel size must be integer or tuple of integer
        bad_values = [(3, "a"), ("a", 3), 2.4, -1, 0]
        for bad_value in bad_values:
            self.assertRaises(ValidationError, Conv2d, self.m, 4, 4, bad_value)
        # stride size must be integer or tuple of integer
        for bad_value in bad_values:
            self.assertRaises(
                ValidationError, Conv2d, self.m, 4, 4, 3, bad_value
            )
        # stride size must be integer or tuple of integer
        for bad_value in bad_values[:-1]:
            self.assertRaises(
                ValidationError, Conv2d, self.m, 4, 4, 3, 1, bad_value
            )

        # bias must be a bool
        self.assertRaises(ValidationError, Conv2d, self.m, 4, 4, 3, bias=10)

    def test_conv2d_load_weights(self):
        conv1 = Conv2d(
            self.m, in_channels=1, out_channels=2, kernel_size=3, bias=True
        )
        conv2 = Conv2d(
            self.m, in_channels=1, out_channels=2, kernel_size=3, bias=False
        )
        ...
        # needs bias as well
        self.assertRaises(ValidationError, conv1.load_weights, w1)

        # conv2 does not have bias
        self.assertRaises(ValidationError, conv2.load_weights, w1, b1)

        # test bad shape
        bad1 = np.random.rand(1)
        self.assertRaises(ValidationError, conv1.load_weights, bad1, b1)
        self.assertRaises(ValidationError, conv1.load_weights, w1, bad1)

        bad2 = np.random.rand(2, 2, 2, 2)
        self.assertRaises(ValidationError, conv1.load_weights, bad2, b1)

        bad3 = np.random.rand(6)
        self.assertRaises(ValidationError, conv1.load_weights, w1, bad3)

        bad4 = np.random.rand(6, 2)
        self.assertRaises(ValidationError, conv1.load_weights, w1, bad4)

    def test_same_indices(self):
        conv1 = Conv2d(self.m, 4, 4, 4, bias=True)
        w1 = np.random.rand(4, 4, 4, 4)
        b1 = np.random.rand(4)
        conv1.load_weights(w1, b1)

        inp = gp.Variable(self.m, domain=dim([4, 4, 4, 4]))
        out, eqs = conv1(inp)

        # this produces an output that is 4 x 4 too
        conv2 = Conv2d(self.m, 4, 4, 4, padding=3, stride=2, bias=True)
        conv2.load_weights(w1, b1)
        out2, eqs2 = conv2(inp)
        conv2.load_weights(w1, b1)

    def test_reloading_weights(self):
        conv1 = Conv2d(
            self.m, in_channels=1, out_channels=2, kernel_size=3, bias=True
        )
        w1 = np.random.rand(2, 1, 3, 3)
        ...
        self.assertTrue(np.allclose(w1, conv1.weight.toDense()))
        self.assertTrue(np.allclose(b1, conv1.bias.toDense()))
    ...
    def test_make_variable(self):
        ...
        w1 = np.random.rand(2, 1, 3, 3)
        b1 = np.random.rand(2)
        self.assertRaises(ValidationError, conv1.load_weights, w1, b1)
        self.assertTrue(isinstance(conv1.weight, gp.Variable))
        self.assertTrue(isinstance(conv1.bias, gp.Variable))
        inp = gp.Variable(self.m, domain=dim([4, 1, 4, 4]))
        out, eqs = conv1(inp)

    def test_load_weight_make_var(self):
        conv1 = Conv2d(
            self.m, in_channels=1, out_channels=2, kernel_size=3, bias=True
        )
        w1 = np.random.rand(2, 1, 3, 3)
        b1 = np.random.rand(2)
        conv1.load_weights(w1, b1)
        self.assertTrue(isinstance(conv1.weight, gp.Parameter))
        self.assertTrue(isinstance(conv1.bias, gp.Parameter))
        self.assertRaises(ValidationError, conv1.make_variable)

    def test_call_bad(self):
        conv1 = Conv2d(self.m, 4, 4, 4, bias=True)
        inp = gp.Variable(self.m, domain=dim([4, 4, 4, 4]))
        # requires initialization before
        self.assertRaises(ValidationError, conv1, inp)
        ...
        # needs 4 dimension
        bad_inp = gp.Variable(self.m, domain=dim([4, 4, 4]))
        self.assertRaises(ValidationError, conv1, bad_inp)

        # in channel must match 4
        # batch x in_channel x height x width
        bad_inp_2 = gp.Variable(self.m, domain=dim([10, 3, 4, 4]))
        self.assertRaises(ValidationError, conv1, bad_inp_2)

    def test_simple_correctness(self):
        conv1 = Conv2d(self.m, 1, 2, 3)
        conv1.load_weights(self.w1, self.b1)
        par_input = gp.Parameter(
            self.m, domain=dim(self.inp.shape), records=self.inp
        )
        ...
        self.assertTrue(np.allclose(out.toDense(), expected_out))

    def test_with_padding(self):
        conv1 = Conv2d(self.m, 1, 2, 3, padding=(2, 1))
        conv1.load_weights(self.w1, self.b1)
        par_input = gp.Parameter(
            self.m, domain=dim(self.inp.shape), records=self.inp
        )
    ...
    def test_with_stride(self):
        conv1 = Conv2d(self.m, 1, 2, 3, stride=(2, 1))
        conv1.load_weights(self.w1, self.b1)
        par_input = gp.Parameter(
            self.m, domain=dim(self.inp.shape), records=self.inp
        )
    ...
    def test_with_padding_and_stride(self):
        conv1 = Conv2d(self.m, 1, 2, 3, stride=(2, 1), padding=(1, 2))
        conv1.load_weights(self.w1, self.b1)
        par_input = gp.Parameter(
            self.m, domain=dim(self.inp.shape), records=self.inp
        )
...
def nn_formulation_suite():
    suite = unittest.TestSuite()
    tests = [
        NNFormulationSuite(name)
        for name in dir(NNFormulationSuite)
...
if __name__ == "__main__":
    ...
    runner.run(nn_formulation_suite())
