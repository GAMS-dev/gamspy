...
def validation_off():
    ...
    old = gp.get_option("VALIDATION")
    gp.set_options({"VALIDATION": 0})
    yield
    gp.set_options({"VALIDATION": old})


def test_get_domain_path_with_string_domain(m):
    s = Set(m, "s", domain=["someLabel"])

    # a relaxed domain given as a bare label terminates the walk
    assert validation.get_domain_path(s) == ["someLabel", "s"]


def test_get_domain_path_with_alias(m):
    i = Set(m, "i", records=["a"])
    al = Alias(m, "al", i)
...
def test_get_domain_path_with_subset(m):
    i = Set(m, "i", records=["a"])
    j = Set(m, "j", domain=[i])
...
def test_index_components_reinserts_literals(m):
    i = Set(m, "i", records=["a"])
    j = Set(m, "j", records=["x"])
    t = Set(m, "t", records=["t1"])
    two_d = Set(m, "two_d", domain=[i, j])
    target = Parameter(m, "target", domain=[i, j, t])
...
def test_expand_leaf_for_unvalidatable_leaf(m):
    u = UniverseAlias(m, "u")
...
def test_expand_leaf_for_relaxed_implicit_set(m):
    s = Set(m, "s", domain=["*", "*"])
...
def test_validate_equations_skips_mpsge_models(m):
    i = Set(m, "i", records=["a"])
    e = Equation(m, "e", domain=[i])
    model = Model(m, "hansen", equations=[e], problem="mcp")
    ...
    m._mpsge_models.append("hansen")
    assert validation.validate_equations(model) is None


def test_get_def_file_special_cases(m):
    sysdir = m.system_directory
...
def test_validate_solver_options_missing_file(m):
    missing = os.path.join(m.working_directory, "does_not_exist.opt")

    with pytest.raises(RuntimeError, match="Error while reading"):
        validation.validate_solver_options(m.system_directory, missing, "cplex")


def test_validate_solver_options_rejects_unknown_option(m):
    path = os.path.join(m.working_directory, "bad.opt")
    with open(path, "w") as file:
        file.write("this is not a valid option\n")

    with pytest.raises(ValidationError, match="Unknown option"):
        validation.validate_solver_options(m.system_directory, path, "cplex")


def test_validate_solver_options_with_bad_system_directory(m):
    path = os.path.join(m.working_directory, "empty.opt")
    with open(path, "w") as file:
        file.write("")

    with pytest.raises(RuntimeError):
        validation.validate_solver_options(
            os.path.join(m.working_directory, "not_a_sysdir"), path, "cplex"
        )


def test_validate_solver_options_skipped_without_validation(m, validation_off):
    missing = os.path.join(m.working_directory, "does_not_exist.opt")

    assert (
        validation.validate_solver_options(m.system_directory, missing, "cplex") is None
    )
